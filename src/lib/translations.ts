      ...
      projects: [
        {
          ...
          y: "2026",
          description: "<strong>WorkHub</strong> — двусторонний маркетплейс услуг: заказчики публикуют задачи, исполнители откликаются, общаются в чате в реальном времени и обмениваются оценками. Полноценный продукт в продакшене — пользовательское приложение, отдельная админ-панель и Telegram-сторож для мониторинга сервера.\n\n<strong>Технологии:</strong> React 19, TanStack Router/Query, React Hook Form, Zod, Express.js, PostgreSQL, Drizzle ORM, JWT (httpOnly cookies), Яндекс Карты, Dadata, Nginx, PM2.\n\n<strong>Проблема:</strong> разрозненный поиск исполнителей, небезопасные сделки и отсутствие единого пространства для общения и оценок.\n<strong>Решение:</strong> единая платформа с регистрацией по email-коду, геопоиском заказов на карте, чатом в реальном времени, рейтингами, дневными лимитами и защитой от мошенничества.\n\n<strong>✔ Результат:</strong> запущен в продакшен на собственном VPS (nginx + TLS, PM2, бэкапы). Масштабируемая архитектура с отдельной админкой (модерация, статистика, управление категориями) и Telegram-мониторингом (алерты о падениях, диске, бэкапах, SSL). Реализованы email-уведомления, блокировка пользователей и юридический контур.",
          liveLink: "https://workhub.su",
        },
        {
          n: "02",
      ...
      projects: [
        {
          ...
          y: "2026",
          description: "<strong>WorkHub</strong> — a two-sided services marketplace: customers post tasks, providers respond, chat in real time, and exchange ratings. A complete product in production — a user app, a separate admin panel, and a Telegram watchdog for server monitoring.\n\n<strong>Technologies:</strong> React 19, TanStack Router/Query, React Hook Form, Zod, Express.js, PostgreSQL, Drizzle ORM, JWT (httpOnly cookies), Yandex Maps, Dadata, Nginx, PM2.\n\n<strong>Problem:</strong> fragmented search for providers, unsafe deals, and no single space for communication and ratings.\n<strong>Solution:</strong> a unified platform with email-code registration, geolocation order search on a map, real-time chat, ratings, daily limits, and fraud protection.\n\n<strong>✔ Result:</strong> launched in production on a dedicated VPS (nginx + TLS, PM2, backups). Scalable architecture with a separate admin panel (moderation, statistics, category management) and Telegram monitoring (alerts for downtime, disk, backups, SSL). Email notifications, user blocking, and a legal framework are implemented.",
          liveLink: "https://workhub.su",
        },
        {
          n: "02",
